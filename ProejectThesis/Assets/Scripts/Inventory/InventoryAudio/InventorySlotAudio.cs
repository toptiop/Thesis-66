...
public class InventorySlotAudio : MonoBehaviour, IDropHandler, IDragHandler, IBeginDragHandler, IEndDragHandler, IPointerClickHandler, IPointerEnterHandler, IPointerExitHandler
{
    [Header("Inventory Detail")]
    public InventoryAudio inventory;
    ...
    public void OnDrop(PointerEventData eventData)
    {
        ...
        {
            InventorySlotAudio slot = eventData.pointerDrag.GetComponent<InventorySlotAudio>();
            if (slot != null)
            {
                if (slot.item == item)
                {
    ...
    public void SwapSlot(InventorySlotAudio newSlot)
    {
        SO_Item keepItem;
        int keepStack;
    ...
    public void MerageThisSlot(InventorySlotAudio mergeSlot)
    {
    ...
    public void MerageThisSlot(SO_Item mergeItem, int mergeAmout)
    {
        ...
        if (amontLeft > 0)
        {
            InventorySlotAudio slot = inventory.IsEmptySlotLeft(mergeItem, this);
            if (slot == null)
            {
    ...
    public void SetThisSlot(SO_Item newItem, int amount)
    {
        ...
        if (amontLeft > 0)
        {
            InventorySlotAudio slot = inventory.IsEmptySlotLeft(newItem, this);
            if (slot == null)
            {
                //Drop Item
