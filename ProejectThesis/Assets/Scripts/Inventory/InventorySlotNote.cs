...
public class InventorySlotNote : MonoBehaviour, IDropHandler, IDragHandler, IBeginDragHandler, IEndDragHandler, IPointerClickHandler, IPointerEnterHandler, IPointerExitHandler
{
    ...
    public void OnDrop(PointerEventData eventData)
    {
        ...
        {
            InventorySlotNote slot = eventData.pointerDrag.GetComponent<InventorySlotNote>();
            if (slot != null)
            {
                if (slot.item == item)
                {
    ...
    public void MerageThisSlot(InventorySlotNote mergeSlot)
    {
    ...
    public void MerageThisSlot(SO_Item mergeItem, int mergeAmout)
    {
        ...
        if (amontLeft > 0)
        {
            InventorySlotNote slot = inventory.IsEmptySlotLeft(mergeItem, this);
            if (slot == null)
            {
    ...
    public void SetThisSlot(SO_Item newItem, int amount)
    {
        ...
        if (amontLeft > 0)
        {
            InventorySlotNote slot = inventory.IsEmptySlotLeft(newItem, this);
            if (slot == null)
            {
                //Drop Item
